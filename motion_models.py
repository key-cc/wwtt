...
import sys
from motion_cnn import resnet152,resnet101

class Encoder(nn.Module):
    def __init__(self, latent_dim,inchannels=2):
        super(Encoder, self).__init__()
        resnet = resnet152(pretrained=True,channel=inchannels)
        self.feature_extractor = nn.Sequential(*list(resnet.children())[:-1])
        self.final = nn.Sequential(
            nn.Linear(resnet.fc_custom.in_features, latent_dim),
...
class ConvLSTM(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(latent_dim,inchannels=2)
...
if __name__ == "__main__":
    jim_input = torch.randn((2,16,2,224,224))

    model = ConvLSTM(num_classes=101, latent_dim=512)
