...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        u_image_paths = sorted(
            glob.glob(sequence_path + os.sep +"u"+os.sep+"*.jpg"),
            key=lambda path: self._frame_number(path),
        )
        v_image_paths = sorted(
            glob.glob(sequence_path + os.sep +"v"+os.sep+"*.jpg"),
            key=lambda path: self._frame_number(path),
        )

        # Pad frames sequences shorter than `self.sequence_length` to length
        u_image_paths = self._pad_to_length(u_image_paths)
        v_image_paths = self._pad_to_length(v_image_paths)

        if self.training:
            # Randomly choose sample interval and start frame
            sample_interval = np.random.randint(
                1, len(u_image_paths) // self.sequence_length + 1
            )
            start_i = np.random.randint(
                0, len(u_image_paths) - sample_interval * self.sequence_length + 1
            )
            ...
            sample_interval = (
                ...
                else len(u_image_paths) // self.sequence_length
            )
            flip = False

        # Extract frames as tensors
        image_sequence = []
        for i in range(start_i, len(u_image_paths), sample_interval):
            image_tensor = torch.FloatTensor(2, 224, 224)
            # Append up to sequence_length
            if (
                self.sequence_length is None
                or len(image_sequence) < self.sequence_length
            ):
                u_image = Image.open(u_image_paths[i])
                v_image = Image.open(v_image_paths[i])

                u_image_tensor = self.transform(u_image)
                v_image_tensor = self.transform(v_image)
                image_tensor[0, :, :] = u_image_tensor
                image_tensor[1, :, :] = v_image_tensor
...
if __name__ == "__main__":
    ...
    print("ok")
    train_dataloader = DataLoader(train_dataset, batch_size=2, drop_last=True,shuffle=True, num_workers=0)
    count=0
